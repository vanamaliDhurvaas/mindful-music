//
//  AfricanViewController.swift
//  MusicApp
//
//  Created by Vanamali Dhurvaas on 7/14/22.
//

import UIKit
import AVFAudio

class AfricanViewController: UIViewController, UITableViewDelegate, UITableViewDataSource
{
    ...
    override func viewDidLoad()
    {
        ...
        var value = UserDefaults.standard.integer(forKey: "hiddenMinData2")
        ...
        var value2 = UserDefaults.standard.integer(forKey: "hiddenSecData2")
        ...
        var value3 = UserDefaults.standard.integer(forKey: "hiddenHourData2")
        if value3 != nil
        {
            hiddenHours = value3
...
//        let fileUrl = url.appendingPathComponent("updatingDateFolder").appendingPathComponent("africanDate.txt")
...
//                           attributes: [FileAttributeKey.creationDate: Date()])
        ...
        let fileUrl = folderUrl.appendingPathComponent("airDate.txt")
        do {
...
//        }
    ...
    func configureSongs()
    {
        songs.append(Song(name: "Calm African Music",
                          albumName: "African Sounds",
                          artistName: "Mindful Music",
                          imageName: "africaPic",
                          trackname: "calmAfrica"))
        
        songs.append(Song(name: "African Folk Music",
                          albumName: "African Sounds",
                          artistName: "Mindful Music",
                          imageName: "africaPic",
                          trackname: "folk"))
        
        songs.append(Song(name: "Relaxing African Music",
                          albumName: "African Sounds",
                          artistName: "Mindful Music",
                          imageName: "africaPic",
                          trackname: "relax"))
        
        songs.append(Song(name: "On the Serengeti",
                          albumName: "African Sounds",
                          artistName: "Mindful Music",
                          imageName: "africaPic",
                          trackname: "serengeti"))
        
        songs.append(Song(name: "Sounds Of Africa",
                          albumName: "African Sounds",
                          artistName: "Mindful Music",
                          imageName: "africaPic",
                          trackname: "soundAfrica"))
    }
    ...
    {
        let cell = tableView.dequeueReusableCell(withIdentifier: "CellId11") as! UITableViewCell
    ...
    @objc func didChangeSwitch(_ sender: UISwitch!)
    {
        ...
        {
            let urlString = Bundle.main.path(forResource: "calmAfrica", ofType: "mp3")
            do
            {
        ...
        {
            let urlString = Bundle.main.path(forResource: "folk", ofType: "mp3")
            do
            {
                try AVAudioSession.sharedInstance().setMode(.default)
                try AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
        ...
        {
            let urlString = Bundle.main.path(forResource: "relax", ofType: "mp3")
            do
            {
                try AVAudioSession.sharedInstance().setMode(.default)
                try AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
        ...
        {
            let urlString = Bundle.main.path(forResource: "serengeti", ofType: "mp3")
            do
            {
                try AVAudioSession.sharedInstance().setMode(.default)
                try AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
        ...
        {
            let urlString = Bundle.main.path(forResource: "soundAfrica", ofType: "mp3")
            do
            {
                try AVAudioSession.sharedInstance().setMode(.default)
                try AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
    ...
    @objc func didTapTimerStartButton()
    {
        ...
        timer2 = Timer.scheduledTimer(timeInterval: 1,
                                      target: self,
                                      selector: #selector(AirSoundsViewController.timerClass2),
                                      userInfo: nil,
                                      repeats: true)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if (segue.identifier == "finishSegue")
        {
            ...
            else
            {
                ...
                player7?.stop()
                player8?.stop()
                let word3 = hiddenMinutesLabel.text!
                let word4 = hiddenHoursLabel.text!
                let nvc = segue.destination as! HomeViewController
                nvc.passedWord3 = word3
                nvc.passedWord4 = word4
                UserDefaults.standard.set(hiddenMinutesLabel.text, forKey: "myYogaMinData")
                UserDefaults.standard.set(hiddenSecondsLabel.text, forKey: "myYogaSecData")
                UserDefaults.standard.set(hiddenHoursLabel.text, forKey: "myYogaHourData")
                
                UserDefaults.standard.set(hiddenHours, forKey: "hiddenHourData2")
                UserDefaults.standard.set(hiddenMinutes, forKey: "hiddenMinData2")
                UserDefaults.standard.set(hiddenSeconds, forKey: "hiddenSecData2")
                timer2.invalidate()
            }
